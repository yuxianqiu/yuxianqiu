        ...
        <el-row  style="margin-top: 20px;">
            <table >
                <tr>
                    ...
                    <th class="titBg"><span v-if="res.la1!='0'" class="block" style="margin-top: 10px;">未分配</span><div v-if="res.la1!='0'" style="display: flex;justify-content: center;"><div  class="ellipse" @click="changeShow">{{res.la1}}</div></div>
                ...
                <tr v-for="(data,index) in formData" :key="index">
                    ...
                    <!--                 <th ><div class="color1 addYs" v-if="data.wpf2">{{data.wpf2}}</div><div v-if="data.wpf" :class="{color2:data.wpf!=''}" class="addYs">{{data.wpf}}</div></th>-->
                    <th @dragenter="dragenter($event,index)"
                        @dragover="dragover($event,index)"
                        @dragleave="dragleave($event,index)"
                        @drop="drop($event,index)">
                        <div v-if="formData1[index].wpf.indexOf(',')!='-1'" :cs="'2'">
                            <div  draggable="true" :cs="'2'" @dragstart="tableDragstart($event,index)" @ondrag="tableDrag($event,index)"  @dragend="tableDragend($event,index)" v-for="(item,index1) in formData1[index].wpf.split(',')" class="addYs" :key="index1" :class="{color1:item.split('_')[1]=='1','color2':item.split('_')[1]=='2'}">{{item.split('_')[0]}}</div>
                        </div>
                        <div v-else draggable="true" :cs="'2'" @dragstart="tableDragstart($event,index)" @ondrag="tableDrag($event,index)"  @dragend="tableDragend($event,index)" class="addYs"  :class="{color1:formData1[index].wpf.split('_')[1]=='1','color2':formData1[index].wpf.split('_')[1]=='2'}">{{formData1[index].wpf.split('_')[0]}}</div>
                    </th>
                    <th   draggable="true"
                          @dragstart="tableDragstart($event,index)" @ondrag="tableDrag($event,index)"  @dragend="tableDragend($event,index)"
                          :cs="'3'" @dragenter="dragenter($event,index)"
                          @dragover="dragover($event,index)"
                          @dragleave="dragleave($event,index)"
                          @drop="drop($event,index)" :class="{color4:formData1[index].SA_cdn.split('_')[1]=='0','color1':formData1[index].SA_cdn.split('_')[1]=='1','color2':formData1[index].SA_cdn.split('_')[1]=='2'}" class="addYs">{{formData1[index].SA_cdn.split('_')[1]=='0'?'请假':formData1[index].SA_cdn.split('_')[0]}}</th>
                    <th
                            draggable="true"
                            @dragstart="tableDragstart($event,index)" @ondrag="tableDrag($event,index)"  @dragend="tableDragend($event,index)"
                            ...
                            @dragover="dragover($event,index)"
                            @dragleave="dragleave($event,index)"
                            @drop="drop($event,index)" :class="{color4:formData1[index].SA_mrx.split('_')[1]=='0','color1':formData1[index].SA_mrx.split('_')[1]=='1','color2':formData1[index].SA_mrx.split('_')[1]=='2'}" class="addYs">{{formData1[index].SA_mrx.split('_')[1]=='0'?'请假':formData1[index].SA_mrx.split('_')[0]}}</th>
                    <th draggable="true"
                        @dragstart="tableDragstart($event,index)" @ondrag="tableDrag($event,index)"  @dragend="tableDragend($event,index)"
                        :cs="'5'" @dragenter="dragenter($event,index)"
                        @dragover="dragover($event,index)"
                        @dragleave="dragleave($event,index)"
                        @drop="drop($event,index)" :class="{color4:formData1[index].SA_oyyn.split('_')[1]=='0','color1':formData1[index].SA_oyyn.split('_')[1]=='1','color2':formData1[index].SA_oyyn.split('_')[1]=='2'}" class="addYs">{{formData1[index].SA_oyyn.split('_')[1]=='0'?'请假':formData1[index].SA_oyyn.split('_')[0]}}</th>
                    <th draggable="true"
                        @dragstart="tableDragstart($event,index)" @ondrag="tableDrag($event,index)"  @dragend="tableDragend($event,index)"
                        :cs="'6'" @dragenter="dragenter($event,index)"
                        @dragover="dragover($event,index)"
                        @dragleave="dragleave($event,index)"
                        @drop="drop($event,index)" :class="{color4:formData1[index].SA_ww.split('_')[1]=='0','color1':formData1[index].SA_ww.split('_')[1]=='1','color2':formData1[index].SA_ww.split('_')[1]=='2'}" class="addYs">{{formData1[index].SA_ww.split('_')[1]=='0'?'请假':formData1[index].SA_ww.split('_')[0]}}</th>
                    <th  draggable="true"
                         @dragstart="tableDragstart($event,index)" @ondrag="tableDrag($event,index)"  @dragend="tableDragend($event,index)"
                         :cs="'7'" @dragenter="dragenter($event,index)"
                         @dragover="dragover($event,index)"
                         @dragleave="dragleave($event,index)"
                         @drop="drop($event,index)"   :class="{color4:formData1[index].SA_zwh.split('_')[1]=='0','color1':formData1[index].SA_zwh.split('_')[1]=='1','color2':formData1[index].SA_zwh.split('_')[1]=='2'}" class="addYs">{{formData1[index].SA_zwh.split('_')[1]=='0'?'请假':formData1[index].SA_zwh.split('_')[0]}}</th>
                </tr>
            </table>
        </el-row>
        <el-row v-if="show"  class="ysShow" style="margin-top: 20px;" >
            <i class="el-icon-circle-close ysShow_close" @click="closeShow"></i>
            <!--       <div v-for="(item,index) in tkData" :key="index"  @dragstart="drag($event)" @dragover="allowDrop($event)" :class="{ bg1: item.pd=='yes', 'bg2': item.pd!='yes' }" draggable="true"  >{{item.name}}</div>-->
            <div v-for="(item,index) in tkData" :key="index"  @dragstart="dragstart($event,index)" @ondrag="drag($event,index)"  @dragend="dragend($event,index)" class="bg" :class="{ color1: item.name.split('_')[1] == '1', 'color2': item.name.split('_')[1] == '2' }" draggable="true"  >{{item.name.split('_')[0]}}</div>
        </el-row>
        ...
        data() {
            return {
                ...
                formData1:[    //过滤之后的数据
                    ...
                    {wpf:'闽A123456_2,京B56369_1',SA_cdn:'',SA_mrx:'',SA_oyyn:'',SA_ww:'',SA_zwh:''},
                    {wpf:'',SA_cdn:'',SA_mrx:'',SA_oyyn:'',SA_ww:'',SA_zwh:''},
                    {wpf:'',SA_cdn:'',SA_mrx:'',SA_oyyn:'',SA_ww:'',SA_zwh:''},
                    {wpf:'',SA_cdn:'',SA_mrx:'',SA_oyyn:'',SA_ww:'',SA_zwh:''},
                    ...
                    {wpf:'',SA_cdn:'',SA_mrx:'',SA_oyyn:'',SA_ww:'',SA_zwh:''},
                    {wpf:'',SA_cdn:'',SA_mrx:'',SA_oyyn:'',SA_ww:'',SA_zwh:''},
                    {wpf:'',SA_cdn:'_',SA_mrx:'',SA_oyyn:'',SA_ww:'',SA_zwh:''},
                    {wpf:'',SA_cdn:'',SA_mrx:'',SA_oyyn:'',SA_ww:'',SA_zwh:''},
                    ...
                    if (this.table_x == '2'){   //  拖出的是第二行的
                        ...
                        if (this.index_x == '2') {   // 拉入第二行 （同）
                            ...
                            if (this.formData[this.index_x].fp2 == this.formData[this.index_x].fp1) { // 如果第二行满的情况
                                ...
                                if (this.formData1[this.index_y][key1].indexOf(',')!='-1') { // 有两个的情况
                                    let a = this.formData1[this.index_y][key1].split(',')
                                    let b = []
                                    a.forEach(x =>{
                                        b.push(x)
